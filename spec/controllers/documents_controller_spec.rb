...
describe DocumentsController do
  ...
  context "GET 'new'" do
    ...
    it "assigns a privacy policy object" do
      get :new
      assigns(:privacy_policy).should be_a PrivacyPolicy
    end
  end
  ...
  context "POST create" do
    context "when creation is successful" do
      it "creates a new terms of service document" do
        ...
        expect { post :create, :documents => { :terms_of_service => { :document => tos_file } } }.to change { TermsOfService.count }.by(1)
      end

      it "creates a new privacy policy document" do
        privacy_policy_file = fixture_file_upload('/documents/placeholder.pdf')
        expect { post :create, :documents => { :privacy_policy => { :document => privacy_policy_file } } }.to change { PrivacyPolicy.count }.by(1)
      end

      it "redirects to the index page" do
        tos_file = fixture_file_upload('/documents/placeholder.pdf')
        post :create, :documents => { :terms_of_service => { :document => tos_file } }
    ...
    context "when the creation is unsuccessful" do
      it "doesn't create a new terms of service document" do
        expect { post :create, :documents => { :terms_of_service => { :document => nil } } }.not_to change { TermsOfService.count }
      end

      it "doesn't create a new privacy policy document" do
        expect { post :create, :documents => { :privacy_policy => { :document => nil } } }.not_to change { PrivacyPolicy.count }
      end

      it "re-renders the new page with errors" do
        post :create, :documents => { :terms_of_service => { :document => nil } }
  ...
  context "GET 'index'" do
    ...
    it "assigns the newest privacy policy" do
      new_privacy_policy = Timecop.freeze(2.days.ago) { FactoryGirl.create(:privacy_policy) }
      old_privacy_policy = Timecop.freeze(10.days.ago) { FactoryGirl.create(:privacy_policy) }
      get :index
      assigns(:privacy_policy).should == new_privacy_policy
    end
  end
end
